# PPC hanging rootogram

Convenience wrapper for PPC rootogram plots.

## Usage

``` r
ppc_rootogram(x, ...)
```

## Arguments

- x:

  A PPC object.

- ...:

  Additional arguments passed to `.plot_ppc`.

## Value

Invisibly returns the plot object.

## Details

Implementation of `ppc_rootogram`. Refer to the package vignettes and
the cited references for a complete algorithmic and mathematical
description.

## See also

[`plot.demonoid.ppc`](https://robustecologies.github.io/lucifer/reference/plot.demonoid.ppc.md)

## Examples

``` r
if (FALSE) { # \dontrun{
## see package vignettes for full examples involving ppc_rootogram
} # }
```
