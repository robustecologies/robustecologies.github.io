...
cat("Gram matrix for 4D orthogonal cone:\n")
#> Gram matrix for 4D orthogonal cone:
print(G_4d)
#>      [,1] [,2] [,3] [,4]
#> [1,]    1    0    0    0
#> [2,]    0    1    0    0
...
cat("\nGram matrix for non-orthogonal 3D cone:\n")
#> 
#> Gram matrix for non-orthogonal 3D cone:
print(round(G_3d, 4))
#>     v1  v2 v3
...
cat(sprintf("This encodes the angle information:\n"))
#> This encodes the angle information:
cat(sprintf("  v1 . v2 = %.4f (angle = 60 deg)\n", G_3d[1, 2]))
#>   v1 . v2 = 0.5000 (angle = 60 deg)
cat(sprintf("  v1 . v3 = %.4f (angle = 90 deg)\n", G_3d[1, 3]))
#>   v1 . v3 = 0.0000 (angle = 90 deg)
cat(sprintf("  v2 . v3 = %.4f (angle = 90 deg)\n", G_3d[2, 3]))
#>   v2 . v3 = 0.0000 (angle = 90 deg)
```
...
cat("Associated matrix for 4D orthant:\n")
#> Associated matrix for 4D orthant:
print(M_orth)
#>      [,1] [,2] [,3] [,4]
#> [1,]    1    0    0    0
#> [2,]    0    1    0    0
...
cat("\n\nAssociated matrix for non-orthogonal 3D cone:\n")
#> 
#> 
#> Associated matrix for non-orthogonal 3D cone:
print(round(M_nonorth, 4))
#>      [,1] [,2] [,3]
#> [1,]  1.0 -0.5    0
#> [2,] -0.5  1.0    0
#> [3,]  0.0  0.0    1
cat(sprintf("\nIs positive definite: %s\n", is_positive_definite(M_nonorth)))
#> 
#> Is positive definite: TRUE
cat(sprintf("Eigenvalues: %s\n",
...
cat("Random 3D cone:\n")
#> Random 3D cone:
cat(sprintf("  Associated matrix is PD: %s\n", is_pd))
#>   Associated matrix is PD: FALSE

if (!is_pd) {
  cat("  Eigenvalues: ", paste(round(eigen(M_random, symmetric = TRUE)$values, 4),
                               collapse = ", "), "\n")
}
...
cat(sprintf("  Solid angle (decomposition): %.6f\n", omega_decomp))
...
#### Add omega method                                                    ####
omega_result <- omega(V_test)
results_3d <- c(results_3d, omega_mvn = omega_result$omega)
...
cat("Tetrahedron face solid angle:\n")
#> Tetrahedron face solid angle:
cat(sprintf("  Van Oosterom formula: %.6f\n", omega_face))
#>   Van Oosterom formula: 0.125000
cat(sprintf("  As fraction of sphere: %.2f%%\n", omega_face * 100))
...
result_eco <- omega(alpha)

cat("Ecological feasibility domain:\n")
#> Ecological feasibility domain:
cat(sprintf("  Species: %d\n", S))
#>   Species: 4
cat(sprintf("  Feasibility volume (Omega): %.6f\n", result_eco$omega))
#>   Feasibility volume (Omega): 0.001288
cat(sprintf("  Per-species measure: %.6f\n", result_eco$omega_scaled))
#>   Per-species measure: 0.189439
cat(sprintf("  Interpretation: %.2f%% of parameter space supports coexistence\n",
            result_eco$omega * 100))
#>   Interpretation: 0.13% of parameter space supports coexistence
```
