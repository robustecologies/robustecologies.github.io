...
[`compute_series_term_3d()`](https://robustecologies.github.io/SolidAngleR/reference/compute_series_term_3d.md).
...
**Implementation:** `R/omega.R` exports `omega(alpha)`, returning both
the raw measure and the geometric mean per dimension \\\omega^{1/n}\\.
...
### R layer
    ...
    R/
      ...
      main_interface.R             compute_solid_angle, diagnose_cone, S3 methods
      ...
      omega.R                      Multivariate normal integration
...
The main entry point `compute_solid_angle(V, method = "auto")`
implements a decision tree:

    V (n x n matrix)
        |
        +---> normalize_vectors(V)
        +---> is_linearly_independent(V)?  --No--> warning
        |
        +---> method == "auto"?
              |
              +---> n == 2 or n == 3?  -------> formula
              ...
              +---> is_positive_definite(M)?
                    |
                    +--Yes--> is_tridiagonal(V^T V)?
                    |         |
                    |         +--Yes--> tridiagonal series
                    |         +--No---> hypergeometric series
                    |
                    +--No---> decomposition

For forced method selection (`method = "formula"`, `"series"`,
`"tridiagonal"`, `"decomposition"`), validation checks are applied
(e.g., the formula method errors for \\n \> 3\\; the series method
errors if \\M_n(C)\\ is not positive definite).
...
Three S3 methods provide the standard inspection interface:
...
- **[`plot()`](https://rdrr.io/r/graphics/plot.default.html)** renders a
  barplot of eigenvalues colored by sign (blue for positive, red for
...
Returns `list(omega, omega_scaled)` where `omega_scaled = omega^(1/n)`.
Requires `mvtnorm`.
...
function computes \\\kappa(B)\\ exactly via `kappa(B, exact = TRUE)` and
warns when \\\kappa \> 10^{10}\\. The `summary.cone_diagnosis` method
classifies stability using thresholds at \\\kappa \in \\10, 100, 10^6,
10^{10}\\\\.
