...
# Using log="y" because values decay exponentially
plot(results$Dimension, results$Exact_Theory, 
     type = "n", 
     log = "y",
     xlab = "Dimension (n)", 
     ylab = expression(paste("Normalized solid angle (", omega, ")")),
     main = "Computed series vs. Exact theory",
     sub = "Simplicial orthogonal cones (Orthants)",
     xaxt = "n")

# Custom X axis for integers
axis(1, at = dims)
grid(nx = NA, ny = NULL, col = "lightgray", lty = "dotted")

# 1. Exact Theory: Dark Green Line
lines(results$Dimension, results$Exact_Theory, 
      col = "darkgreen", lwd = 3)

# 2. Computed Series: Orange Points
points(results$Dimension, results$Computed_Series, 
       col = "orange", pch = 19, cex = 1.5)

# Add legend
legend("topright", 
       legend = c("Exact theory (1/2^n)", "Computed series"),
...
# Error Trend: Force linear y-axis for error to see machine precision jitter
plot(results$Dimension, results$Error,
     type = "b", # Both points and lines
     col = "red",
     pch = 17,   # Triangle point style
     lwd = 2,
     cex = 1.5,
     xlab = "Dimension (n)",
     ylab = "Absolute error",
     main = "Numerical error trend",
     sub = "Floating point precision check",
     xaxt = "n")

axis(1, at = dims)
grid(nx = NA, ny = NULL, col = "lightgray", lty = "dotted")

# Add zero reference line (if errors are distinguishable from 0)
abline(h = 0, col = "gray", lty = 2)
```

![](f.hypergeometric-high-dim_files/figure-html/validation-1.png)
...
cat("Is associated matrix Positive Definite?", is_pd, "\n")
#> Is associated matrix Positive Definite? TRUE
...
cat(sprintf("Exact solid angle (Series): %.10f\n", true_omega))
#> Exact solid angle (Series): 0.0242383501
cat(sprintf("Terms computed: %d\n", exact_result$n_terms))
#> Terms computed: 1000
```
...
# Plotting
plot(log10(sample_sizes), mc_estimates, type = "b", pch = 19, col = "blue",
     ylim = range(c(mc_estimates - 2*mc_errors, mc_estimates + 2*mc_errors, true_omega)),
     xlab = "log10(Sample Size)", ylab = "Normalized Solid Angle",
     main = "Convergence: Exact Series vs. Monte Carlo (5D)")

# Add Error Bars
arrows(log10(sample_sizes), mc_estimates - 2*mc_errors,
       log10(sample_sizes), mc_estimates + 2*mc_errors,
       angle = 90, code = 3, length = 0.05, col = "blue")

# Add Exact Reference Line
abline(h = true_omega, col = "red", lwd = 2, lty = 2)
legend("topright", c("Monte Carlo Est.", "Exact Series Value"),
       col = c("blue", "red"), lty = c(1, 2), pch = c(19, NA))
```

![](f.hypergeometric-high-dim_files/figure-html/mc_comparison-1.png)
