...
plot(theta_degrees, omega_values,
     type = "l", lwd = 2, col = "blue",
     xlab = expression(paste("Apex angle ", theta, " (degrees)")),
     ylab = expression(paste("Normalized solid angle ", omega)),
     main = "Circular cone solid angle vs. apex angle",
     ylim = c(0, 1))
grid()

# Add some reference lines
abline(h = c(0.25, 0.5, 0.75), lty = 2, col = "gray")
abline(v = c(45, 90, 135), lty = 2, col = "gray")

# Add annotations
text(90, 0.15, expression(paste("Hemisphere (", theta, " = 90", degree, ")")), cex = 0.8)
points(90, 0.5, pch = 19, col = "red")
```

![](c.mazonka-geometric-methods_files/figure-html/circular-cone-examples-1.png)
...
cat("Case 1: Two narrow cones with small separation\n")
#> Case 1: Two narrow cones with small separation
fig_case1
```
...
cat("Case 2: Wide cones with moderate separation\n")
#> Case 2: Wide cones with moderate separation
fig_case2
```
...
plot(phi_values * 180 / pi, omega_intersection,
     type = "l", lwd = 2, col = "darkblue",
     xlab = expression(paste("Separation angle ", phi, " (degrees)")),
     ylab = "Intersection solid angle (normalized)",
     main = bquote("Intersection vs. separation (" * theta[1] * "=" *
                    .(sprintf("%.0f", theta1*180/pi)) * degree * ", " *
                    theta[2] * "=" * .(sprintf("%.0f", theta2*180/pi)) * degree * ")"))
grid()

# Add individual cone solid angles for reference
omega1 <- solid_angle_cone(theta1)
omega2 <- solid_angle_cone(theta2)

abline(h = omega1, lty = 2, col = "red", lwd = 1)
abline(h = omega2, lty = 2, col = "blue", lwd = 1)

text(20, omega1, bquote("Cone 1 (" * theta[1] * "=" *
     .(sprintf("%.0f", theta1*180/pi)) * degree * ")"),
     pos = 3, cex = 0.8, col = "red")
text(20, omega2, bquote("Cone 2 (" * theta[2] * "=" *
     .(sprintf("%.0f", theta2*180/pi)) * degree * ")"),
     pos = 1, cex = 0.8, col = "blue")
```

![](c.mazonka-geometric-methods_files/figure-html/parameter-exploration-1.png)
...
cat("Lighting scenario:\n")
#> Lighting scenario:
cat(sprintf("  Light cone angle: %.0f degrees\n", light_apex_angle * 180 / pi))
#>   Light cone angle: 45 degrees
cat(sprintf("  Light solid angle: %.4f (%.2f%% of sphere)\n",
            light_solid_angle, light_solid_angle * 100))
#>   Light solid angle: 0.1464 (14.64% of sphere)
cat(sprintf("  Surface distance: %.1f m\n", surface_distance))
#>   Surface distance: 5.0 m
cat(sprintf("  Surface solid angle from light: %.6f\n", surface_solid_angle))
...
cat(sprintf("  Estimated form factor: %.4f\n", form_factor))
