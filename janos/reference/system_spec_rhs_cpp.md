...
Translate the symbolic right-hand sides of a formula-based `system_spec`
...
- model:
  ...
  A `system_spec` object built from a formula list, either as a
...
A list of class `system_spec_rhs_cpp` with the following entries:
...
The function consumes the parsed formulas stored on the `system_spec`
...
rhs <- system_spec_rhs_cpp(duffing)
rhs$rhs
#> [1] "y[1]"
