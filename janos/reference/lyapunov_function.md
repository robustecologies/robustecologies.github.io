...
- model:
  ...
  same internal envelope that a `system_spec` would produce after
...
- x_star:
  ...
  for general `system_spec` inputs whose RHS is parsed.
...
# Full system_spec ingress (the canonical form)
m <- system_spec(
