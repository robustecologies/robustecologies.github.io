...
For the specialised problem of detecting Wada basins of attraction and
resolving fractal basin boundaries in dissipative systems, the companion
package [wadaR](https://github.com/robustecologies/wadaR) accepts a
`system_spec` directly through its `compiled_system()` constructor.
Internally, wadaR calls the exported helper
[`system_spec_rhs_cpp()`](https://robustecologies.github.io/janos/reference/system_spec_rhs_cpp.md)
to translate the symbolic right-hand sides into C++ expressions and
inlines them into its parallel OpenMP basin-classification kernel; no
rewriting of equations into raw C++ is required from the user. The same
`system_spec` object thus drives both single-trajectory simulation in
janos and basin classification in wadaR.
...
if (!is.null(system_for_basins)) print(system_for_basins)
```
