...
catalogue of twenty-two solver backends covering deterministic ODEs,
...
layer**
([`system_spec()`](https://robustecologies.github.io/janos/reference/system_spec.md),
`noise_spec()`, and the twenty-two `solver_*()` constructors) provides
...
A `system_spec` object is a list with a stable set of slots that
...
|:---|:---|:---|:---|
| Specification | system_spec.R + noise_spec.R + solvers.R | 19 (1 spec + 4 noise + 17 solvers - overlap) | Declare dynamics and noise |
...
R files and the entirety of the exported API surface. {.table
style="width:100%;"}
...
The `system_spec` contract is also the integration point between janos
and downstream packages that need to reuse a system under a different
code-generation template. The exported helper
[`system_spec_rhs_cpp()`](https://robustecologies.github.io/janos/reference/system_spec_rhs_cpp.md)
walks the formulas of a deterministic ODE or discrete-map `system_spec`
with the same AST translator used by the internal compilers and returns
a list with character entries for each per-state right-hand side, the
target convention (`"deriv"` for ODEs, `"next_state"` for maps), the
ordered state and parameter names, the default parameter values, and the
model meta. State references appear as `y[i]` and parameter references
as `parms[i]` (both 0-based), time as `t`, and the constant `pi` as
`M_PI`, matching the inlining contract of janos’s own RK4, RK45, and
Rosenbrock templates. Stochastic, delayed, jump-diffusion, spatial,
switched, and continuous-time Markov chain families are rejected with an
explicit message, as are models built with `positive_states` clamps or
function-mode `rhs`, because their right-hand sides do not reduce to a
single per-state expression vector.

The companion package [wadaR](https://github.com/robustecologies/wadaR)
consumes this hook from its `compiled_system()` constructor: when a
[`janos::system_spec`](https://robustecologies.github.io/janos/reference/system_spec.md)
is supplied through the `model` argument, wadaR’s internal
`adapt_system_spec()` adapter calls
[`system_spec_rhs_cpp()`](https://robustecologies.github.io/janos/reference/system_spec_rhs_cpp.md),
rewrites the `y[i]` indexing to wadaR’s `state[i]` convention and
parameter indices to bare names matched by the `const double`
declarations of the basin template, and inlines the result into the
parallel OpenMP basin-classification kernel. The same `system_spec` thus
drives both single-trajectory simulation in janos and Wada-basin
classification in wadaR without restating the equations. A worked
example sits in
...
A `system_spec` is the unique integration point between user intent and
...
reserved name, or one of these tokens. {.table}

### The eleven model types

The `type` field of a `system_spec` is determined by inspecting which
...
environment, and attaches them to the `system_spec` object via
...
| solver_bdf() | ODE (stiff) | Backward differentiation formula (orders 1-5) | 1-5 | O(N_t \* d^3) with Jacobian reuse |
| solver_esdirk() | ODE (stiff) | TR-BDF2 ESDIRK (Hosea-Shampine 1996) | 2 | O(N_t \* d^3) with single Jacobian factorisation per step |
| solver_imex_ark() | ODE (stiff slow-fast split) | Additive IMEX-RK ARS(2,3,2) (Ascher-Ruuth-Spiteri 1997) | 2 | O(N_t \* d^3) on the implicit block only |
| solver_radau() | ODE (stiff) | Radau IIA (s=2, order 3) with block-Newton | 3 | O(N_t \* (s d)^3) with block-Newton at stage system |
...
adaptively from the propensity distribution. {.table}
...
Five exported utilities support the stiff-ODE workflow.
`slow_fast_partition(model, method = c("eigen", "graph", "user"))`
linearises the right-hand side at a reference state, partitions the
state vector into a fast block and a slow block via spectral-gap
analysis of the Jacobian, and returns an S3 object with `fast_idx`,
`slow_idx`, `tau_fast`, `tau_slow`, `coupling_norm` and the eigenvalue
spectrum. `stiffness_ratio(model)` returns the ratio of largest to
smallest absolute real eigenvalue of the Jacobian, together with the
spectral abscissa and the field-of-values radius; the ratio is the
canonical stiffness indicator and gates the choice between explicit,
semi-implicit Rosenbrock and fully implicit Radau IIA.
`gsp_reduce(model, partition, order = 0L)` applies the zeroth-order
Tikhonov geometric singular perturbation reduction, slaving the fast
variables to the slow via Newton iteration on the fast residual and
returning a reduced-RHS closure over the slow block; a first-order
Fenichel correction can be supplied as a user closure. `expm_pade(A, t)`
and `expm_krylov(A, v, t, m)` evaluate the matrix exponential by Higham
2005 scaling-and-squaring and by Arnoldi-Krylov projection respectively;
both feed exponential integrators and the linearised stiffness
diagnostic.
...
static shared library shipped with the package contains the
Bartels-Stewart routine used by the Lyapunov subsystem
(`src/lyapunov_bartels_stewart.cpp`), the four stiff-solver engines
(`src/stiff_bdf.cpp`, `src/stiff_esdirk.cpp`, `src/stiff_imex.cpp`,
`src/stiff_radau.cpp`) sharing the header
`inst/include/janos_stiff.hpp`, and the `R_init_janos()` registration
stub. Every generated C++ file uses `arma::vec` for state vectors,
`arma::ivec` for integer populations in SSA, `arma::mat` and
...
| model | system_spec | Back-pointer to the model spec that produced it |
...
these slots; none of them are considered internal. {.table}
...
janos exports thirty-two S3 classes covering models, simulations,
analyses, Lyapunov reports, and chaos diagnostics. Every class
implements the full `print`, `summary` and `plot` trio.
[`print()`](https://rdrr.io/r/base/print.html) is a one-screen synopsis
returning the object invisibly.
...
|:----------------------|:-----:|:-------:|:----:|
| system_spec           |  yes  |   yes   | yes  |
| noise_spec            |  yes  |   yes   | yes  |
| solver_spec           |  yes  |   yes   | yes  |
| dyn_sim               |  yes  |   yes   | yes  |
| ensemble_sim          |  yes  |   yes   | yes  |
| observed_dyn_sim      |  yes  |   yes   | yes  |
| equilibrium_point     |  yes  |   yes   | yes  |
...
| correlation_dimension |  yes  |   yes   | yes  |
| poincare_section      |  yes  |   yes   | yes  |
| zero_one_test         |  yes  |   yes   | yes  |
| bifurcation_diagram   |  yes  |   yes   | yes  |

S3 class method matrix. Every class carries the full print/summary/plot
triad. {.table}
...
consume the `system_spec` contract, adding new capability is a matter of
...
|:---|:---|:---|
| system_spec() | Declare a dynamical system (11 types) | system_spec |
| system_spec_rhs_cpp() | Extract per-state C++ RHS strings (interop hook) | system_spec_rhs_cpp |
