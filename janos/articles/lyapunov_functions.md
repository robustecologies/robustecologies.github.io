...
ecological models. The Bayesian extension
`lyapunov_function_posterior()` computes, for each posterior sample of
the community parameters, whether a Lyapunov function exists, yielding
the robustness index \\\text{RI} = P(\text{Lyapunov exists} \mid
\text{data})\\ as a posterior probability of structural stability.
...
motivates the Bayesian extension in `lyapunov_function_posterior()`, but
the computational cost of per-sample certification can be substantial
for large \\K\\.
...
conclusion is fragile. The janos package provides
`lyapunov_function_posterior()` for Bayesian Lyapunov certification: it
evaluates Lyapunov function existence for each of \\K\\ posterior
samples and computes the robustness index \\\text{RI} = P(\text{Lyapunov
exists} \mid \text{data})\\. See the janos documentation for details.
...
Auto-dispatch method selection by system type and dimension.
...
## Lyapunov analysis over any `model_spec`

The eight methods above all operate on the lightweight
`dynamical_system` class introduced in section *Computational methods*.
janos also exposes a higher-level layer,
[`analyse_lyapunov()`](https://robustecologies.github.io/janos/reference/analyse_lyapunov.md),
that accepts the canonical
[`model_spec()`](https://robustecologies.github.io/janos/reference/model_spec.md)
object and applies family-specific theory automatically: discrete
Lyapunov equations for maps, stochastic Lyapunov theorems for SDEs,
...
inputs to arbitrary
[`model_spec()`](https://robustecologies.github.io/janos/reference/model_spec.md)
specifications is not merely a quality-of-life bridge. It is a
reorganisation of Lyapunov theory around the idea that the mathematical
object one writes down (ODE, map, SDE, DDE, PDMP, CTMC, PDE) dictates
what a Lyapunov function even means: for an ODE it is a classical
\\C^1\\ function decreasing along trajectories; for an SDE it is a
\\C^2\\ function whose Ito generator \\\mathcal{L}V\\ is negative; for a
DDE it is a functional on path segments rather than a point function;
for a PDMP it is a common quadratic that survives every switch; for a
CTMC it is a Foster function whose generator drift is bounded above by
\\-\epsilon V + b\\\mathbf{1}\_C\\; for a PDE it is a functional on
function space that descends along semigroup trajectories. The common
thread is the existence of a scalar observable whose evolution is
one-signed along the dynamics; the differences are what “evolution” and
“observable” mean in each category.
...
`analyse_lyapunov(model_spec)` therefore does three things in strict
...
layer reads the boolean flags of the `model_spec`: `is_sde`, `is_dde`,
...
### ODE models via the bridge

[`analyse_lyapunov()`](https://robustecologies.github.io/janos/reference/analyse_lyapunov.md)
on an ODE model invokes
[`lyapunov_function()`](https://robustecologies.github.io/janos/reference/lyapunov_function.md)
under the hood, after converting the `model_spec` into a
`dynamical_system` via structural detectors. No ecological subtlety is
lost: a VL-stable competitive gLV still yields a global Goh logarithmic
certificate.
...
implements this lift: it constructs the fluid-limit `model_spec`,
invokes
[`analyse_lyapunov()`](https://robustecologies.github.io/janos/reference/analyse_lyapunov.md)
...
The extension of the Lyapunov module to `model_spec` brings three tiers
