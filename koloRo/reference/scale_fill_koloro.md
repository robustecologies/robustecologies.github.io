# Discrete fill scale using koloRo palettes

Apply a koloRo palette as a discrete fill scale in ggplot2.

## Usage

``` r
scale_fill_koloro(palette = "okabe_ito", direction = 1, ...)
```
...
ggplot(mtcars, aes(x = factor(cyl), fill = factor(cyl))) +
  geom_bar() +
  scale_fill_koloro("alhambra_nazari")
} # }
```
