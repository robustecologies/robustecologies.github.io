# Plots of posterior predictive checks for iterquad

This may be used to plot, or save plots of, samples in an object of
class `iterquad.ppc`. A variety of plots is provided.
...
- x:
  ...
  This required argument is an object of class `iterquad.ppc`.
...
- Rows:
  ...
  records associated by row in the object of class `iterquad.ppc`. Only
  these rows are plotted. The default is to plot all rows. Some plots do
  not allow rows to be specified.
