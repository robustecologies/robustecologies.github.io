...
\\\mathbb{R}^2\\. This metric measures how far two sets are from being
identical and is widely used in shape comparison and boundary analysis.
...
- \\d_H(X, Y) \geq 0\\ (non-negativity)

- \\d_H(X, Y) = 0\\ iff \\X = Y\\ (identity of indiscernibles)

- \\d_H(X, Y) = d_H(Y, X)\\ (symmetry)

- \\d_H(X, Z) \leq d_H(X, Y) + d_H(Y, Z)\\ (triangle inequality)
