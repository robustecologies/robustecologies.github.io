...
classifying each grid point by its winning-coalition outcome.
...
Two possible outcomes based on which species coalition survives: Basin 1
corresponds to species {1, 2, 3} surviving, while Basin 2 corresponds to
species {1, 4, 5} surviving. The boundary between these outcomes is
fractal but not Wada (only 2 basins).

**8-species scenario:**

Three outcomes based on which invading species (6, 7, or 8) dominates:
Basin 1 corresponds to species 6 dominating, Basin 2 to species 7
dominating, and Basin 3 to species 8 dominating. With 3+ basins, the
system can exhibit Wada basin boundaries.
...
t_max=2000 and dt=0.01, this means 8 billion ODE steps. Use resolution
50-100 for exploration, increase to 200-400 for publication-quality
figures, and note that the 8-species scenario is slower due to the
invasion phase.
