...
- n_points:
  ...
  pair. Default is 5000. Higher values give more accurate saddle
  approximation but increase computation time. Start with 1000-2000 for
  exploration and increase to 5000+ for final results.
...
- max_iter:
  ...
  Integer. Maximum iterations for trajectory evolution. Default is 3000.
  Higher values ensure trajectories converge to attractors, especially
  for points near basin boundaries. For the forced damped pendulum, use
  at least 3000 iterations for reliable results.
...
each of the \\N_A\\ merged basin pairs, the algorithm must find initial
straddling points via random sampling, integrate two trajectories
forward while collecting saddle points, and perform repeated bisection
refinements.

The total number of ODE integration steps scales approximately as
\\O(N_A \times n\\points \times T/\Delta t)\\. For 3 basins with
`n_points=10000`, this can take several minutes to hours. Use
`n_points = 1000-2000` for exploration, increase to
`n_points = 5000-10000` for accurate saddle approximation, and consider
using
...
for faster Wada detection when dynamical insight is not needed.
...
# ===================================================================== #
# IMPORTANT: The saddle-straddle method requires careful parameter tuning.
# Key parameters for successful saddle detection:
#   - max_iter: Must be high enough for trajectories to converge (>= 2000)
#   - dt: Small enough for accurate integration (0.01 or smaller)
#   - n_points: More points = better saddle coverage but slower
#
pendulum <- forced_damped_pendulum(forcing = 1.66)

# Use extended max_iter for reliable convergence
...
# ===================================================================== #
# Example 4: Why saddle-straddle can be difficult                       #
# ===================================================================== #
# The method has several challenges:
# 1. Random search must find points in different merged basins
# 2. Trajectories must converge within max_iter * dt time
# 3. The chaotic saddle tracking can escape if basins are very interleaved
#
# Tips for reliable results:
# - Use max_iter >= 3000 for the pendulum
# - Start with n_points = 1000-2000 for testing
# - Increase to n_points = 5000+ for publication quality
#
