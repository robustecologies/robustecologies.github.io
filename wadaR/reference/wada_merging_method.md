...
The basins are classified as Wada if:

- The relative difference \\(d\_{\max} - d\_{\min}) / d\_{\min} \< 2\\

- The maximum distance normalized by phase space size \\d\_{\max} / L \<
  0.05\\

These thresholds are empirically calibrated from the original paper.
