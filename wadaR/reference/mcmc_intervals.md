...
Convenience wrapper for `plot(x, type = "intervals", ...)`.

## Usage

``` r
mcmc_intervals(x, Parms = NULL, ground_truth = NULL, ...)
```
...
Implementation of `mcmc_intervals`. Refer to the package vignettes and
...
``` r
if (FALSE) { # \dontrun{
## see package vignettes for full examples involving mcmc_intervals
} # }
```
