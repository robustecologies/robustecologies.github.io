# Posterior predictive checks for Variational Bayes
...
is predicted given an object of class `vb`, the model specification, and
data. This function requires that posterior samples were produced with
...
This function returns an object of class `vb.ppc` (where ppc stands for
...
Since Variational Bayes characterizes marginal posterior distributions
with modes and variances, and posterior predictive checks involve
samples, the `predict.vb` function requires the use of independent
samples of the marginal posterior distributions, provided by
[`VariationalBayes`](https://robustecologies.github.io/lucifer/reference/VariationalBayes.md)
when `sir=TRUE`.
...
vb_fit <- VariationalBayes(Model, parm, Data, Covar = NULL,
  Iterations = 1000, Method = "Salimans2", Stop.Tolerance = 1e-3)

# Predict
pred <- predict(vb_fit, Model, Data)
summary(pred)
} # }
```
