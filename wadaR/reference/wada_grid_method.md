...
- attractors:
  ...
  'center' (numeric vector) and 'radius' (numeric). Required for
  refinement with ODE integration.
...
- damping:
  ...
  Numeric. Damping coefficient for ODE integration during refinement.
  Default is 0.2 (standard for forced damped pendulum).

- forcing:

  Numeric. Forcing amplitude for ODE integration during refinement.
  Default is 1.66 (Wada regime for forced damped pendulum).

- dt:

  Numeric. Time step for ODE integration. Default is 0.01.

- t_max:

  Numeric. Maximum integration time for ODE. Default is 100.

- n_cores:

  Integer. Number of CPU cores for parallel computation. Default is NULL
  (auto-detect, using all cores minus one).
...
speeds up computation at the cost of accuracy. Use
`max_refinements = 10` for quick screening, `max_refinements = 20`
(default) for accurate results, and for very large grids consider using
