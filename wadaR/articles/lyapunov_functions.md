...
ecological models. A Bayesian extension that evaluates, for each
posterior sample of the community parameters, whether a Lyapunov
function exists, yielding the robustness index \\\text{RI} =
P(\text{Lyapunov exists} \mid \text{data})\\ as a posterior probability
of structural stability, may be added in a future release.
...
motivates a per-sample certification pass over the posterior; its
computational cost can be substantial for large \\K\\, and an explicit
Bayesian extension may be added in a future release.
...
conclusion is fragile. A Bayesian Lyapunov certification pass, which
would evaluate Lyapunov function existence for each of \\K\\ posterior
samples and return the robustness index \\\text{RI} = P(\text{Lyapunov
exists} \mid \text{data})\\, may be added in a future release.
...
[`lyapunov_function()`](https://robustecologies.github.io/janos/reference/lyapunov_function.md)
directly on the `model_spec`. The structural detectors
